...
contract LoanTest is Test {
    ...
    function setUp() public {
        ...
        nativeVault = new Vault(address(aero), address(loan), "40op-AERO-VAULT", "40op-AERO-VAULT");
        ...
        loan.setNativeVault(address(nativeVault));
        loan.setNativeLtv(4000); // 40% LTV for native loans
        ...
        // Mint AERO to native vault
        vm.startPrank(0x40EbC1Ac8d4Fedd2E144b75fe9C0420BE82750c6); // AERO minter
        aero.transfer(address(nativeVault), 1000e18);
        vm.stopPrank();
    }
    ...
    function testGetMaxLoan() public {
        uint256 veBalance = votingEscrow.balanceOfNFTAt(tokenId, block.timestamp);
        uint256 calcLoan = (veBalance * 4000) / 10000;
        uint256 loanWithFee = calcLoan * 10000 / (10000 + 80);

        (uint256 maxLoan,  ) = loan.getMaxLoan(tokenId, address(aero));
        assertEq(maxLoan, loanWithFee);
        vm.startPrank(user);
        IERC721(address(votingEscrow)).approve(address(loan), tokenId);
        uint256 amount = 0.1e18;
        uint256 originationFee = (amount * 80) / 10000;
        loan.requestLoan(tokenId, amount, Loan.ZeroBalanceOption.DoNothing, 0, address(0), false, false, address(aero));

        (maxLoan,  ) = loan.getMaxLoan(tokenId, address(aero));
        uint256 expectedMaxLoan = loanWithFee - amount - originationFee;

        assertEq(maxLoan, expectedMaxLoan);

        loan.increaseLoan(tokenId, 0.1e18);
        (maxLoan,  ) = loan.getMaxLoan(tokenId, address(aero));
        expectedMaxLoan = expectedMaxLoan - 0.1e18 - originationFee;
        assertEq(maxLoan, expectedMaxLoan);
    ...
    function testLoanRequest() public {
        ...
        assertEq(aero.balanceOf(address(nativeVault)), 1000e18);
        vm.startPrank(user);
        IERC721(address(votingEscrow)).approve(address(loan), tokenId);
        uint256 amount = 5e18;
        vm.expectRevert();
        ...
        loan.requestLoan(tokenId, amount, Loan.ZeroBalanceOption.DoNothing, 0, address(0), false, false, address(aero));
        vm.stopPrank();
        assertTrue(aero.balanceOf(address(user)) > 0.1e18);
        assertTrue(aero.balanceOf(address(nativeVault)) < 1000e18);
    ...
    function xtestLoanVotingAdvance() public {
        ...
        assertEq(aero.balanceOf(address(nativeVault)), 999.9e18, "Vault should have 999.9e18");
    ...
    //     vm.startPrank(user);
    //     IERC721(address(votingEscrow)).approve(address(loan), tokenId);
    //     loan.requestLoan(tokenId, amount, Loan.ZeroBalanceOption.InvestToVault, 0, address(0), false, false, address(usdc));
    //     vm.roll(block.number+1);
    //     loan.setIncreasePercentage(tokenId, 10000);
    //     vm.stopPrank();
    //     assertEq(usdc.balanceOf(address(owner)), startingOwnerBalance, "Owner should have starting balance");
    ...
    //     CommunityRewards(address(_proxy)).initialize(address(loan), tokens, 0, 318, 0xeBf418Fe2512e7E6bd9b87a8F0f294aCDC67e6B4, address(usdc));
    //     vm.stopPrank();
    //     vm.startPrank(owner);
    ...
    //     loan.requestLoan(524, 0, Loan.ZeroBalanceOption.InvestToVault, 0, address(0), false, false, address(usdc));
    //     vm.roll(block.number+1);
    ...
    function testLoanFullPayoff() public {
        ...
        aero.transfer(address(user), 1000e18);
        vm.stopPrank();
        ...
        loan.requestLoan(tokenId, amount, Loan.ZeroBalanceOption.DoNothing, 0, address(0), false, false, address(aero));
        vm.stopPrank();
        assertEq(aero.balanceOf(address(user)), 0.1e18+startingUserBalance, "User should have 1e6");
        assertEq(aero.balanceOf(address(nativeVault)), 999.9e18, "Loan should have 97e6");

        assertEq(votingEscrow.ownerOf(tokenId), address(loan));

        vm.startPrank(user);
        aero.approve(address(loan), 0.2e18);
        loan.pay(tokenId, 0);
    ...
    //     _loan.requestLoan(_tokenId, 0, Loan.ZeroBalanceOption.PayToOwner, 0, address(0), false, false, address(aero));
    //     vm.stopPrank();
    ...
    //     uint256 beginningUserAeroBalance = aero.balanceOf(address(user));
    //     address[] memory bribes = new address[](0);
    ...
    //     uint256 endingUserAeroBalance = aero.balanceOf(address(user));
    //     assertTrue(endingUserAeroBalance > beginningUserAeroBalance, "User USDC balance should increase");
    ...
    //     vm.startPrank(user);
    //     IERC721(address(votingEscrow)).approve(address(_loan), _tokenId);
    //     _loan.requestLoan(_tokenId, 0, Loan.ZeroBalanceOption.PayToOwner, 0, 0x940181a94A35A4569E4529A3CDfB74e38FD98631, false, false, address(usdc));
    //     vm.stopPrank();
    //     (uint256 balance,) = _loan.getLoanDetails(_tokenId);
    //     assertEq(balance, 0, "should have no balance");
    ...
    //     loan.requestLoan(_tokenId, 0, Loan.ZeroBalanceOption.InvestToVault, 0, address(0), false, false, address(usdc));
    //     vm.roll(block.number+1);
    //     vm.stopPrank();
    ...
    //     vm.startPrank(user);
    //     IERC721(address(votingEscrow)).approve(address(loan), _tokenId);
    //     loan.requestLoan(_tokenId, 0, Loan.ZeroBalanceOption.PayToOwner, 0, address(0), false, false, address(usdc));
    //     vm.roll(block.number+1);
    ...
    //     vm.roll(block.number + 1);
    //     CommunityRewards(address(_proxy)).initialize(address(loan), tokens, 2500e18, _tokenId, 0xeBf418Fe2512e7E6bd9b87a8F0f294aCDC67e6B4, address(usdc));
    ...
    function testMerge() public {
        ...
        vm.startPrank(_user);        
        IERC721(address(votingEscrow)).approve(address(loan), _tokenId);
        loan.requestLoan(_tokenId, 0, Loan.ZeroBalanceOption.PayToOwner, 0, address(0), true, false, address(aero));
        vm.stopPrank();
    ...
    //     vm.startPrank(user);
    //     IERC721(address(votingEscrow)).approve(address(loan), _tokenId);
    //     loan.requestLoan(_tokenId, loanAmount, Loan.ZeroBalanceOption.PayToOwner, 0, address(0), false, false, address(usdc));
    ...
    //     loan.requestLoan(_tokenId2, loanAmount, Loan.ZeroBalanceOption.PayToOwner, 0, address(0), false, false, address(usdc));
    ...
    //     vm.roll(block.number + 1);
    //     IERC721(address(votingEscrow)).approve(address(loan), _tokenId2);
    //     loan.requestLoan(_tokenId2, loanAmount, Loan.ZeroBalanceOption.PayToOwner, 0, address(0), true, false, address(usdc));
    //     loan.setPayoffToken(_tokenId2, true);
    ...
    function testManualVoting() public {
        uint256 _tokenId = 524;
        vm.startPrank(0x40EbC1Ac8d4Fedd2E144b75fe9C0420BE82750c6);
        aero.transfer(address(nativeVault), 10000e18);
        vm.stopPrank();
        uint256 amount = 0.1e18;
        address _user = votingEscrow.ownerOf(_tokenId);
        ...
        loan.requestLoan(_tokenId, amount, Loan.ZeroBalanceOption.DoNothing, 0, address(0), false, false, address(aero));
        vm.warp(block.timestamp + 1);
        vm.roll(block.number + 1);
