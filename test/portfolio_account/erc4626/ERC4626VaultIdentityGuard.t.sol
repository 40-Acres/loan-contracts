...
contract ERC4626VaultIdentityGuardTest is Test {
    ...
    function test_mismatch_pay_reverts() public {
        vm.prank(_owner);
        _config.setCollateralVault(address(_vaultA));

        _registerFacets(address(_vaultB));

        // pay() reaches decreaseTotalDebt -> _snapshotIfNeededRepay (guarded).
        // Debt is 0 so the pulled amount caps to 0; the guard must still fire.
