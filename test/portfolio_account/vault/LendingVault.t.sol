 ...
 *
 * The fix in LendingVault.borrowFromPortfolio:
 *   1. Reverts with ExceedsUtilization when totalAssets() == 0 (previously the
 *      ternary in the old computation treated this as 0% util and any borrow passed).
 *   2. Uses strict inequality in multiplication form:
 *        if (postBorrowLoaned * 10000 >= maxUtilizationBps * total) revert
 *      so a borrow that lands at *exactly* the cap is rejected.
 *
 * The withdraw side is intentionally NOT gated by maxUtilizationBps — the cap
 * applies to new lending only, not to LP exits. The regression tests below
 * pin that design.
 */
contract LendingVaultUtilizationCapTest is Test {
    LendingVault public vault;
    MockUSDC public usdc;
    MockPortfolioFactory public portfolioFactory;
    ...
    uint256 constant MAX_UTIL_BPS = 8000; // 80%
    uint256 constant ORIG_FEE_BPS = 0;    // 0% — keep math clean for boundary tests
    ...
    // ─────────────────────────────────────────────────────────────────
    // (1) Zero-supply borrow reverts — previously buggy: borrows passed
    //     freely on an empty vault.
    // ─────────────────────────────────────────────────────────────────
    ...
    function test_borrowFromPortfolio_revertsOnZeroTotalAssets() public {
        ...
        vm.prank(borrower);
        vm.expectRevert(LendingVault.ExceedsUtilization.selector);
        vault.borrowFromPortfolio(1);
    }

    // ─────────────────────────────────────────────────────────────────
    // (3) Strict-inequality boundary:
    //     - 800 USDC borrow lands at exactly 80% util → MUST revert.
    //     - 799 USDC borrow lands just under 80% → MUST succeed.
    // ─────────────────────────────────────────────────────────────────

    function test_borrowFromPortfolio_revertsAtExactCap() public {
        _deposit(depositor1, 1000e6);

        // 800 * 10000 == 8000 * 1000 → strict-inequality (>=) rejects.
        vm.prank(borrower);
        vm.expectRevert(LendingVault.ExceedsUtilization.selector);
    ...
    function test_borrowFromPortfolio_succeedsOneWeiBelowCap() public {
        ...
        // 799 * 10000 = 7_990_000 < 8000 * 1000 = 8_000_000 → passes.
        vm.prank(borrower);
        vault.borrowFromPortfolio(799e6);

        assertEq(vault.getDebtBalance(borrower), 799e6, "debt = 799 USDC");
        assertEq(vault.activeAssets(), 799e6, "totalLoanedAssets = 799 USDC");
    }

    // ─────────────────────────────────────────────────────────────────
    // (5) Regression: withdraw is NOT gated by maxUtilizationBps.
    //     Borrow 70% (under cap), then LP withdraws as much as liquidity
    //     allows, pushing post-withdraw utilization to ~98.6% — far above
    //     the 80% cap. This MUST succeed; the cap is borrow-side only.
    // ─────────────────────────────────────────────────────────────────
    ...
    function test_withdraw_notGatedByUtilizationCap() public {
        ...
        // totalAssets = 1000, loaned = 700 → util = 70% (under 80% cap).

        // Liquid balance is now 1000 - 700 = 300 USDC. LP withdraws 290, leaving
        // 10 liquid + 700 loaned = 710 totalAssets, so utilization becomes
        // 700/710 ≈ 98.6% — far above the 80% cap. This must still succeed.
        uint256 liquidBefore = usdc.balanceOf(address(vault));
        assertEq(liquidBefore, 300e6, "precondition: 300 USDC liquid");
        ...
        // Post-withdraw utilization: 700 loaned / 710 totalAssets > 80% — confirms
        // we are well past the borrow-side cap, and yet the withdraw succeeded.
        uint256 totalAssetsAfter = vault.totalAssets();
        assertEq(totalAssetsAfter, 710e6, "totalAssets = 10 liquid + 700 loaned");
        // 700 * 10000 / 710 = 9859 bps, > 8000 bps cap.
