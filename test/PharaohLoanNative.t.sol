...
import {PharaohDeployNative} from "../script/PharaohDeploy.s.sol";
...
contract PharaohLoanNativeTest is Test {
    uint256 fork;
    uint256 fork2;
    uint256 _fork = vm.createFork(vm.envString("AVAX_RPC_URL"));
    uint256 _fork2 = vm.createFork(vm.envString("AVAX_RPC_URL"));
    ...
    function testMaxLoan() public {
        ...
        assertEq(maxLoan, 75e18);

        loan.increaseLoan(tokenId, 70e18);
        (maxLoan,  ) = loan.getMaxLoan(tokenId);
        assertEq(maxLoan, 5e18);

        loan.increaseLoan(tokenId, 5e18);
        (maxLoan,  ) = loan.getMaxLoan(tokenId);
        assertEq(maxLoan, 0);
        vm.stopPrank();
    ...
    function testLoanRequest() public {
        ...
        assertEq(aero.balanceOf(address(user)), startingUserBalance);
        assertEq(aero.balanceOf(address(vault)), 100e18);
        vm.startPrank(user);
        IERC721(address(votingEscrow)).approve(address(loan), tokenId);
        uint256 amount = 1e18;
        loan.requestLoan(tokenId, amount, Loanv2.ZeroBalanceOption.DoNothing, 0, address(0), false, false);
        vm.stopPrank();
        assertTrue(aero.balanceOf(address(user)) > 1e18);
        assertTrue(aero.balanceOf(address(vault)) < 100e18);
    ...
    function xtestLoanVotingAdvance() public {
        ...
        uint256 startingUserBalance = aero.balanceOf(address(user));
        uint256 startingOwnerBalance = aero.balanceOf(address(owner));
        ...
        vm.stopPrank();
        assertTrue(aero.balanceOf(address(user)) > startingUserBalance, "User should have more than starting balance");
        assertEq(aero.balanceOf(address(vault)), 99e18, "Vault should have 1e18");
        assertEq(aero.balanceOf(address(owner)), startingOwnerBalance, "Owner should have starting balance");
        ...
        assertNotEq(aero.balanceOf(address(owner)), startingOwnerBalance, "owner should have gained");
    ...
    function xtestIncreaseAmountPercentage100NoLoan() public {
        ...
        console.log("LOAN BALANCE", aero.balanceOf(address(loan)));
        ...
        assertEq(aero.balanceOf(address(owner)), startingOwnerBalance, "owner should not have gained");
    ...
    function xtestIncreaseAmountPercentage75NoLoan() public {
        ...
        assertEq(aero.balanceOf(address(owner)), 3345, "owner should have gained");
    ...
    //     address veOwner = votingEscrow.ownerOf(5215);
    ...
    //     vm.roll(block.number + 1);
    //     votingEscrow.approve(address(_proxy), 5215);
    //     CommunityRewards(address(_proxy)).initialize(address(loan), tokens, 0, 5215, 0xAAAEa1fB9f3DE3F70E89f37B69Ab11B47eb9Ce6F);
    //     vm.stopPrank();
    //     vm.startPrank(owner);
    //     loan.setManagedNft(5215);
    ...
    //     bytes memory data = hex"";
    //     uint256[2] memory allocations = [uint256(expectedRewards), uint256(0)];
    //     uint256 rewards = _claimRewards(loan, 5215, bribes, data, allocations);
    //     vm.stopPrank();
    ...
    //     rewards = _claimRewards(loan, 5215, bribes, data, allocations);
    //     uint256 userBalance = aero.balanceOf(address(user));
    //     assertTrue(ownerShares > 0, "owner should have shares");
        
    //     uint256 ownerUsdBalance = aero.balanceOf(address(owner));
    //     vm.roll(block.number + 1);
    //     vm.warp(block.timestamp + 7 days);
    //     vm.prank(address(loan));
    //     communityRewards.notifyRewardAmount(tokens[0], 10e18);
    //     aero.mint(address(communityRewards), 10e18);
    //     communityRewards.getRewardForUser(user, tokens);
    //     communityRewards.getRewardForUser(owner, tokens);

    //     assertTrue(aero.balanceOf(address(owner)) > ownerUsdBalance, "owner should have more than starting balance");
    // }
    ...
    function testIncreaseLoan() public {
        ...
        loan.vote(tokenId);
        vm.stopPrank();
        assertTrue(aero.balanceOf(address(user)) > 1e18, "User should have more than loan");

        assertEq(loan.activeAssets(),1e18, "ff");
        (uint256 balance, address borrower) = loan.getLoanDetails(tokenId);
        assertTrue(balance > amount, "Balance should be 1e18");
        assertEq(borrower, user);
        ...
        assertTrue(balance> amount, "Balance should be more than amount");
        assertEq(borrower, user);
        assertEq(loan.activeAssets(),2e18, "ff");

        assertEq(aero.balanceOf(address(user)), 2e6 + startingUserBalance, "User should have .02e6");
        assertEq(aero.balanceOf(address(vault)), 98e18, "Loan should have 1e18");
        
    }

    function testLoanFullPayoff() public {
        vm.prank(address(votingEscrow));
        aero.transfer(address(user), 100e18);
        vm.stopPrank();
        ...
        uint256 startingUserBalance = aero.balanceOf(address(user));
        assertEq(aero.balanceOf(address(user)), startingUserBalance, "User should have startingUserBalance");
        assertEq(aero.balanceOf(address(vault)), 100e18);
        ...
        vm.stopPrank();
        assertEq(aero.balanceOf(address(user)), 1e18+startingUserBalance, "User should have 1e18");
        assertEq(aero.balanceOf(address(vault)), 99e18, "Loan should have 97e6");

        assertEq(votingEscrow.ownerOf(tokenId), address(loan));

        vm.startPrank(user);
        aero.approve(address(loan), 5e18);
        vm.expectRevert();
    ...
    function xtestReinvestVault() public {
        assertEq(aero.balanceOf(address(vault)), 100e18, "vault should have 0 balance");
        uint256 _tokenId = 5215;

        uint256 startingOwnerBalance = aero.balanceOf(address(owner));
        uint256 startingVaultBalance = aero.balanceOf(address(vault));
        ...
        uint256 endingOwnerBalance = aero.balanceOf(address(owner));

        

        // owner should not receive rewards
        assertNotEq(endingOwnerBalance - startingOwnerBalance, 0, "owner should receive rewards");
        assertTrue(aero.balanceOf(address(vault)) > startingVaultBalance, "vault should have more than starting balance");
    }
    ...
    function xtestPayToOwner() public {
        ...
        uint256 loanBeginningBalance = aero.balanceOf(address(loan));
        user = votingEscrow.ownerOf(_tokenId);
        uint256 startingOwnerBalance = aero.balanceOf(address(Ownable2StepUpgradeable(loan).owner()));
        uint256 startingUserBalance = aero.balanceOf(address(user));
        uint256 startingLoanBalance = aero.balanceOf(address(loan));
        vm.startPrank(user);
        IERC721(address(votingEscrow)).approve(address(loan), _tokenId);
        ...
        uint256 endingOwnerBalance = aero.balanceOf(address(Ownable2StepUpgradeable(loan).owner()));
        uint256 endingLoanBalance = aero.balanceOf(address(loan));
        ...
        uint256 loanEndingBalance = aero.balanceOf(address(loan));
    ...
    function xtestPayToOwnerPayoutToken() public {
        ...
        // owner should receive rewards in aero 
    ...
    //     vm.startPrank(address(loan));
    //     communityRewards.deposit(uint256(5215), 10e18, user1);
    //     communityRewards.deposit(uint256(5215), 10e18, user2);
    //     vm.stopPrank();
    ...
    //     uint256[2] memory allocations = [uint256(expectedRewards), uint256(0)];
    //      _claimRewards(loan, tokenId, bribes, data, allocations);
    //     uint256 rewards = communityRewards.tokenRewardsPerEpoch(address(aero), ProtocolTimeLibrary.epochStart(block.timestamp) - ProtocolTimeLibrary.WEEK);
    //     assertTrue(rewards > 0, "rewards should be greater than 0");
    //     console.log(aero.balanceOf(address(user1)));
    ...
    //     assertTrue(IERC20(address(aero)).balanceOf(address(communityRewards)) <  10, "should be less than 10");
    ...
    function xtestPayoffToken() public {
        // aero.mint(address(vault), 100000e18);
    ...
    function xtestPayoffTokenMoreBalance() public {
        ...
        uint256 _tokenId2 = 5215;


        uint256 loanAmount = 2e18;
    ...
    function testClaimWithIncreasePercentage() public {
        ...
        vm.selectFork(_fork);
        address user2 = votingEscrow.ownerOf(_tokenId);
        vm.startPrank(user2);
        voter.reset(_tokenId);
        votingEscrow.approve(address(loan),_tokenId);
        loan.requestLoan(_tokenId, 0, Loanv2.ZeroBalanceOption.PayToOwner, 1000, address(0), false, false);
        vm.stopPrank();

        IVotingEscrow.LockedBalance memory lockedBalance = votingEscrow.locked(_tokenId);
        int128 lockedAmount = lockedBalance.amount;

        uint256 beginningLoanaeroBalance = aero.balanceOf(address(loan));
        address[] memory bribes = new address[](0);
        bytes memory data = hex"";
        uint256[2] memory allocations = [uint256(51238035086341), uint256(5123803508634)];     
        uint256 rewards = _claimRewards(loan, _tokenId, bribes, data, allocations);
        uint256 endingLoanaeroBalance = aero.balanceOf(address(loan));
        assertEq(endingLoanaeroBalance - beginningLoanaeroBalance, 0, "Loan aero balance should not change");
        vm.roll(block.number + 1);
        vm.warp(block.timestamp + 1);
        lockedBalance = votingEscrow.locked(_tokenId);
        assertGt(lockedBalance.amount, lockedAmount);
    }

    // function testClaimWithIncreasePercentage2() public {
    //     uint256 _tokenId = 5408;
    //     vm.selectFork(_fork);
    //     vm.startPrank(Loan(0xd3E726b681C9a1E2a620cef9fE0EcE49822B11d4).owner());
    //     Loan loanV2 = new Loan();
    //     Loan(0xd3E726b681C9a1E2a620cef9fE0EcE49822B11d4).upgradeToAndCall(address(loanV2), new bytes(0));
    //     vm.stopPrank();
    //     uint256 beginningLoanaeroBalance = aero.balanceOf(address(loan));
    //     address[] memory bribes = new address[](0);
    ...
    //     uint256 rewards = _claimRewards(Loan(0xd3E726b681C9a1E2a620cef9fE0EcE49822B11d4), _tokenId, bribes, data, allocations);
    //     uint256 endingLoanaeroBalance = aero.balanceOf(address(loan));
    // }
