...
contract XPharaohTest is Test {
    ...
    function setUp() public {
        ...
        (XPharaohLoan loanV2, Vault deployedVault, Swapper deployedSwapper, AccountConfigStorage _accountConfigStorage) = deployer.deploy();
...
contract XPharaohMigrationTest is Test {
    ...
    function testMigration() public {
        ...
        // get locked balance of the token
        // int128 lockedBalance = votingEscrow.locked(tokenId).amount;
        // Set up the XPharaohLoan with PortfolioFactory
        xPharaohLoan.setPortfolioFactory(address(portfolioFactory));
        vm.prank(IOwnable(address(accountConfigStorage)).owner());
        accountConfigStorage.setApprovedContract(address(xPharaohLoanProxy), true);
        ...
        PharaohLoanV2(address(loan)).migrateNft(tokenId, address(xPharaohLoan), address(portfolioFactory));
        vm.stopPrank();
