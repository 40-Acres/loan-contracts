...
contract DynamicFeesVault is Initializable, ERC4626Upgradeable, UUPSUpgradeable, Ownable2StepUpgradeable {
    ...
    function totalAssets() public view override returns (uint256) {
        ...
        uint256 lenderPremiumCurrentEpoch = _debtToken.lenderPremiumUnlockedThisEpoch();
        ...
        return _asset.balanceOf(address(this)) + totalLoanedAssets() - principalRepaidCurrentEpoch + lenderPremiumCurrentEpoch;
    }
     ...
     */
    function getPrincipalRepaidForEpoch(uint256 epoch) public view returns (uint256) {
        uint256 assetsUnlocked = _debtToken.totalAssetsUnlocked(epoch);
        // For past epochs, we need to get the final claimed premium
        // For current epoch, use the unlocked premium
        uint256 currentEpoch = ProtocolTimeLibrary.epochStart(block.timestamp);
    ...
    /**
     * @dev Override _deposit to automatically settle previous epoch and rebalance before deposits
     * @dev This ensures totalAssets() is accurate and vault ratio is maintained when users deposit
     */
    function _deposit(address caller, address receiver, uint256 assets, uint256 shares) internal virtual override {
        ...
        _debtToken.rebalance();
    }

    /**
     * @dev Override _withdraw to automatically settle previous epoch and rebalance before withdrawals
     * @dev This ensures totalAssets() is accurate and vault ratio is maintained when users withdraw
     */
    function _withdraw(
        address caller,
        address receiver,
