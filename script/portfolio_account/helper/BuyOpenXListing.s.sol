...
/**
 * @title BuyOpenXListing
 * @dev Helper script to buy a veNFT from the OpenX marketplace via PortfolioManager multicall
 *
 ...
 *
 * Usage:
 * 1. With parameters: forge script script/portfolio_account/helper/BuyOpenXListing.s.sol:BuyOpenXListing --sig "run(uint256)" <LISTING_ID> --rpc-url $RPC_URL --broadcast
 * 2. From env vars: LISTING_ID=1 forge script script/portfolio_account/helper/BuyOpenXListing.s.sol:BuyOpenXListing --sig "run()" --rpc-url $RPC_URL --broadcast
 *
 * Example:
 * forge script script/portfolio_account/helper/BuyOpenXListing.s.sol:BuyOpenXListing --sig "run(uint256)" 123 --rpc-url $BASE_RPC_URL --broadcast
 */
contract BuyOpenXListing is Script {
    using stdJson for string;

    IOpenXSwap constant OPENX = IOpenXSwap(0xbDdCf6AB290E7Ad076CA103183730d1Bf0661112);

    /**
     * @dev Buy an OpenX listing via PortfolioManager multicall
     * @param listingId The OpenX marketplace listing ID
     * @param buyer The buyer's address (portfolio owner)
     */
    function buyOpenXListing(
        uint256 listingId,
        address buyer
    ) internal {
        ...
        bytes4 selector = OpenXFacet.buyOpenXListing.selector;
        address facet = facetRegistry.getFacetForSelector(selector);
        require(facet != address(0), "OpenXFacet.buyOpenXListing not registered in FacetRegistry. Please deploy facets first.");
        ...
        // Get listing details from OpenX
        (
            ,
            ,
            ,
        ...
        require(sold == 0, "Listing already sold");
        require(price > 0, "Invalid listing price");

        console.log("OpenX Listing Details:");
    ...
    function run(
        ...
        buyOpenXListing(listingId, buyer);
        vm.stopBroadcast();
    }

    /**
     * @dev Alternative run function that reads parameters from environment variables
     * Usage: PRIVATE_KEY=0x... LISTING_ID=1 forge script script/portfolio_account/helper/BuyOpenXListing.s.sol:BuyOpenXListing --sig "run()" --rpc-url $RPC_URL --broadcast
     */
...
// Example usage:
// LISTING_ID=123 forge script script/portfolio_account/helper/BuyOpenXListing.s.sol:BuyOpenXListing --sig "run()" --rpc-url $BASE_RPC_URL --broadcast
