...
/**
 * @title IncreaseLock (YieldBasis)
 * @dev Helper script to increase an existing veYB lock via PortfolioManager multicall
 *
 * When increasing a lock, the lock duration is automatically extended to max (4 years).
 *
 * Portfolio is automatically determined from the factory using the owner address (from PRIVATE_KEY).
 *
 * Usage:
 * 1. With parameters: forge script script/portfolio_account/yieldbasis/helper/IncreaseLock.s.sol:IncreaseLock --sig "run(uint256)" <AMOUNT> --rpc-url $ETH_RPC_URL --broadcast
 * 2. From env vars: AMOUNT=1000000000000000000 forge script script/portfolio_account/yieldbasis/helper/IncreaseLock.s.sol:IncreaseLock --sig "run()" --rpc-url $ETH_RPC_URL --broadcast
 *
 * Example:
 * forge script script/portfolio_account/yieldbasis/helper/IncreaseLock.s.sol:IncreaseLock --sig "run(uint256)" 1000000000000000000 --rpc-url $ETH_RPC_URL --broadcast
 */
contract IncreaseLock is Script {
    ...
    function getYBToken() internal view returns (address) {
        return IYieldBasisVotingEscrow(VE_YB).TOKEN();
    }

    /**
     * @dev Increase an existing veYB lock via PortfolioManager multicall
     * @param amount The additional amount of YB tokens to lock (in wei)
    ...
    function increaseLock(
        ...
        uint256 tokenId = IYieldBasisVotingEscrow(VE_YB).tokenOfOwnerByIndex(portfolioAddress, 0);
        console.log("Current locked amount:", IYieldBasisVotingEscrow(VE_YB).locked(portfolioAddress).amount);
        console.log("Current unlock time:", IYieldBasisVotingEscrow(VE_YB).locked(portfolioAddress).end);
        ...
        console.log("veYB lock increased successfully!");
        console.log("Portfolio Address:", portfolioAddress);
        console.log("Additional Amount Locked:", amount);

        // Get new lock info
        console.log("New Total Locked:", IYieldBasisVotingEscrow(VE_YB).locked(portfolioAddress).amount);
        console.log("New Unlock Time:", IYieldBasisVotingEscrow(VE_YB).locked(portfolioAddress).end);
    }
    ...
    function run(uint256 amount) external {
        ...
        increaseLock(amount, owner);
        vm.stopBroadcast();
    }

    /**
     * @dev Alternative run function that reads parameters from environment variables
     * Usage: PRIVATE_KEY=0x... AMOUNT=1000000000000000000 forge script script/portfolio_account/yieldbasis/helper/IncreaseLock.s.sol:IncreaseLock --sig "run()" --rpc-url $ETH_RPC_URL --broadcast
     */
...
// Example usage:
// AMOUNT=1000000000000000000 forge script script/portfolio_account/yieldbasis/helper/IncreaseLock.s.sol:IncreaseLock --sig "run()" --rpc-url $ETH_RPC_URL --broadcast
