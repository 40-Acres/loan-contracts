...
contract DeployVotingEscrowFacet is AccountFacetsDeploy {
    function run() external {
        ...
        VotingEscrowFacet facet = new VotingEscrowFacet(PORTFOLIO_FACTORY, VOTING_ESCROW, VOTER);
        registerFacet(PORTFOLIO_FACTORY, address(facet), getSelectorsForFacet(), "VotingEscrowFacet", false);
        vm.stopBroadcast();
    }

    function deploy(address portfolioFactory, address votingEscrow, address voter) external {
        VotingEscrowFacet newFacet = new VotingEscrowFacet(portfolioFactory, votingEscrow, voter);
    ...
    function getSelectorsForFacet() internal pure override returns (bytes4[] memory) {
        ...
        selectors[3] = VotingEscrowFacet.onERC721Received.selector;
        return selectors;
    }
}
