...
contract PharaohFacet {
    ...
    function pharaohClaimCollateral(address loanContract, uint256 tokenId) external {
    ...
    function pharaohIncreaseLoan(address loanContract, uint256 tokenId, uint256 amount) external {
        ILoan(loanContract).increaseLoan(tokenId, amount);
        address asset = address(ILoan(loanContract)._asset());
        IERC20(asset).transfer(msg.sender, amount);
    }
    
    function pharaohRequestLoan(address loanContract, uint256 tokenId, uint256 amount, ILoan.ZeroBalanceOption zeroBalanceOption, uint256 increasePercentage, address preferredToken, bool topUp, bool optInCommunityRewards) external {
    ...
    function pharaohVote(address loanContract, uint256 tokenId) external returns (bool success) {
        address ve = address(ILoan(loanContract)._ve());
        IERC721(ve).setApprovalForAll(address(loanContract), true);
    ...
    function pharaohUserVote(address loanContract, uint256[] calldata tokenIds, address[] calldata pools, uint256[] calldata weights) external {
        address ve = address(ILoan(loanContract)._ve());
        IERC721(ve).setApprovalForAll(address(loanContract), true);
    ...
    function pharaohClaim(address loanContract, uint256 tokenId, address[] calldata fees, address[][] calldata tokens, bytes calldata tradeData, uint256[2] calldata allocations) external returns (uint256) {
        address ve = address(ILoan(loanContract)._ve());
        IERC721(ve).setApprovalForAll(address(loanContract), true);
