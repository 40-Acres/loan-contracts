 ...
 *      Debt transfer uses the vault's transferDebt to atomically move debt between borrowers.
 */
contract DynamicMarketplaceFacet is BaseMarketplaceFacet {
    ...
    function _addDebt(address config, uint256 amount, uint256 unpaidFees) internal override {
        DynamicCollateralManager.addDebt(config, amount, unpaidFees);
    }

    function _transferDebtAway(address config, uint256 amount, uint256 /* unpaidFees */, address buyer) internal override {
        DynamicCollateralManager.transferDebtToAddress(config, buyer, amount);
    }
