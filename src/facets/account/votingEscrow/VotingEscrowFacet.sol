...
contract VotingEscrowFacet is AccessControl, IERC721Receiver {
    ...
    function merge(uint256 fromToken, uint256 toToken) external {
        ...
        CollateralManager.removeLockedCollateral(fromToken, address(_portfolioFactory.portfolioFactoryConfig()));
        CollateralManager.updateLockedCollateral(address(_portfolioFactory.portfolioFactoryConfig()), toToken, address(_votingEscrow));
